...
static void
    fn_test_scene_life_info_callback(void* context, FNCustomEventWorker event, FNError fn_error) {
    UNUSED(fn_error);
    furi_check(context);
    furi_check(event);
    FNApp* app = context;
...
void fn_test_scene_life_info_on_enter(void* context) {
    ...
    fn_worker_get_life_info_start(
        app->fn_tmp_data, app->worker, fn_test_scene_life_info_callback, app);
}
    ...
    if(event.type == SceneManagerEventTypeBack) {
        ...
        if(event.event == FNCustomEventWorkerDone) {
            ...
            widget_add_text_scroll_element(
                app->widget, 0, 16, 128, 50, furi_string_get_cstr(tmp_string));
