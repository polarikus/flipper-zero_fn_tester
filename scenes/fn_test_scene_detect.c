...
static void
    fn_test_scene_chip_detect_callback(void* context, FNCustomEventWorker event, FNError fn_error) {
    UNUSED(fn_error);
    FNApp* app = context;
...
void fn_test_scene_detect_on_enter(void* context) {
    ...
    fn_worker_fn_detect_start(app->fn_info, app->worker, fn_test_scene_chip_detect_callback, app);
}
