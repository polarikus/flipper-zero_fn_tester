...
MU_TEST(fn_uart_alloc_test) {
    ...
    mu_assert(furi_thread_get_state(fn_uart->thread) == FuriThreadStateStopped, "FN UART thread started!");
...
MU_TEST(fn_uart_start_and_stop_thread_test) {
    ...
    mu_assert(furi_thread_get_state(fn_uart->thread) == FuriThreadStateRunning, "FN UART not started!");
#ifndef FURI_DEBUG
    mu_assert(fn_uart->uart_id == FuriHalUartIdUSART1, "FN UART ID is not FuriHalUartIdUSART1!!!");
#else
    mu_assert(fn_uart->uart_id == FuriHalUartIdLPUART1, "FN UART ID is not FuriHalUartIdLPUART1!!!");
#endif
    fn_uart_stop_thread(fn_uart);
    mu_assert(furi_thread_get_state(fn_uart->thread) == FuriThreadStateStopped, "FN UART not stopped!");
}
...
MU_TEST(fn_uart_trx_test) {
    size_t test_answer_size = 36;//Ожидаемый размер ответа от ФН. Для этой команды он фиксированный
    uint8_t test_fn_answer_len[] = {0x1F, 0x00};//Ожидаемая Длина сообщения ФН, формат LE (СМ документацию к ФН)
    ...
    mu_assert(rx_len == test_answer_size, "rx_len != 36, is fn connected?");//Коллбек отработал, но не получен ответ от ФН
    mu_assert_int_eq(rx_buff[0], 0x04);//Признак начала сообщения
    for (size_t i = 0; i < COUNT_OF(test_fn_answer_len); ++i) {
        mu_assert_int_eq(test_fn_answer_len[i], rx_buff[i + 1]);//Сравниваем длину пакета с ожидаемой
    }
    uint8_t crc[2];
    crc[0] = rx_buff[rx_len-1];
    crc[1] = rx_buff[rx_len-2];
    mu_assert(check_crc(byte_array_to_uint32t_LE(crc), rx_buff, 1, rx_len - 2), "Wrong CRC!!!");//Проверяем целостность пакета с помощью контрольной суммы
